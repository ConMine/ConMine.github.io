...
# TRAITS
####
get_trait_df <- function(data,Sp_name,temp_json){
  #setup traits dataframe
  trait_names <- enframe(unlist(temp_json[[1]]$life_history_traits))$name
  ...
  for(i in 1:length(data)){ #for each database
    if(length(data[[i]]) == 0){
      ...
      source <- enframe(unlist(data[[names(data)[i]]]$life_history_traits))[non_adult_mass_index,]
...
####
get_t_and_c <- function(data,Sp_name,temp_json){
  #setup dataframe
  tc_names <- c(enframe(unlist(temp_json[[1]]$conservation))$name,
                enframe(unlist(temp_json[[1]]$threats))$name)
...
####
get_shiny_data <- function(data,Sp_name,temp_json){
  #setup dataframe
  shiny_names <- c(enframe(unlist(temp_json[[1]]$taxonomy))$name,
                enframe(unlist(temp_json[[1]]$iucn_status))$name,
                enframe(unlist(temp_json[[1]]$habitat))$name,
                enframe(unlist(temp_json[[1]]$population))$name,
                enframe(unlist(temp_json[[1]]$trade))$name)
